...
| loan_payment_id       | Int? (FK LoanPayment, unique) | optional link when expense records a loan installment |
| created_at            | DateTime                |                    |

- **Relations:** `fortnight` (Fortnight), `user` (User?), `wallet` (Wallet?), `category` (Category?), `expense_template` (ExpenseTemplate?), `house` (House?), `loan_payment` (LoanPayment?), `transferAsUser` (Transfer? – when created by a transfer).
...
**Loan** — principal, lender, type (`PERSONAL` | `PAYROLL`), status, payment amount/count/frequency, start date, payment source (`WALLET` | `PAYROLL_DEDUCTION`), optional `source_wallet_id`, `linked_wallet_id`, `income_template_id`. Owner: `user_id` or `house_id`.

**LoanPayment** — `loan_id`, `sequence`, `due_date`, `amount`, `status` (`SCHEDULED` | `PAID` | …), optional `source_wallet_id`, optional linked `Expense` (`Expense.loan_payment_id`, unique).

### 8.2 Schedule generation

- `generateLoanPaymentSchedule` in `src/lib/finance/loan-schedule.ts` builds installments from loan terms.
- `loan.service.ts` persists payments, marks paid, creates/links expenses for wallet outflows, and exposes planner data.

### 8.3 Integration surfaces

| Surface | Role |
|--------|------|
| `/loans`, `/api/loans` | CRUD and listing |
| `/api/loans/planner` | Fortnight/month loan payments for planning UI |
| `/api/loans/payments/[id]` | Mark paid, update payment |
| Dashboard | Upcoming obligations, recent activity |
| Liquidity projection | Scheduled outflows in 180-day forecast |
| `/api/transactions` | Loan payment events in activity log |
| Wallet detail | `LinkedLoansCard` when wallet is payment source |

### 8.4 Expense link

When a wallet-paid installment is recorded as an expense, `Expense.loan_payment_id` points to the `LoanPayment`. Paying or adjusting via expense APIs must stay consistent with `loan.service.ts` wallet accounting.

---

*Last reviewed: May 2026 (includes loans integration and fortnight lifecycle).*
