/**
 * Parser for C&A Efectivo (Préstamo Personal C&A Bradescard) PDF statements.
...
/** Parse a C&A date string like "15/MAR/26" → Date (UTC noon). */
...
export type CaEfectivoParsedMovement = {
  rawLines: string[];
  day: number;
  month: number;
  description: string;
...
export const parseCaEfectivoStatementText = (
  fullText: string,
): CaEfectivoStatementParseResult => {
  const warnings: string[] = [];
  const text = fullText.replace(/\r\n/g, '\n');

  if (!/PR[EÉ]STAMO PERSONAL C&A BRADESCARD/i.test(text)) {
    warnings.push(
      'El PDF no parece ser un estado de cuenta de Préstamo Personal C&A Bradescard.',
    );
  }
  ...
  // Cut date = statement issue date: "Fecha de Corte: 15/MAR/26"
  ...
  // Payment due date: "Fecha Límite de Pago: 08/ABR/26"
  ...
  // Period: "PERÍODO: 16/FEB/26 - 15/MAR/26"
  ...
  const movements: CaEfectivoParsedMovement[] = [];
  ...
  for (const group of groups) {
    ...
    // Only import DISPOSICION EFTVO lines
    if (!/DISPOSICION\s+EFTVO/i.test(combined)) continue;
...
export const extractCaEfectivoStatementText = async (
  buffer: Buffer,
): Promise<string> => {
