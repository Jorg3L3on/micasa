...
function SidebarMenuSkeleton({
  ...
  // Stable pseudo-random width between 50 to 90% based on the optional className.
  const width = React.useMemo(() => {
    const seed = String(className ?? 'sidebar-skeleton');
    let hash = 0;
    for (let i = 0; i < seed.length; i += 1) {
      hash = (hash + seed.charCodeAt(i) * (i + 1)) % 41;
    }
    return `${hash + 50}%`
  }, [className])
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground flex h-10 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 md:h-7 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
