...
const navButtonClass =
  'inline-flex h-10 min-w-[7.5rem] items-center justify-center gap-2 rounded-xl border border-border/60 bg-card/80 px-3 text-sm font-medium text-muted-foreground shadow-sm transition-all duration-200 hover:border-primary/35 hover:bg-muted/45 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45';
...
export function MonthlyPanelTopBar({
  ...
  return (
    <div
      className={cn(
        'relative overflow-hidden rounded-2xl border border-border/60 bg-card/95 px-3 py-3 shadow-sm',
        'before:pointer-events-none before:absolute before:inset-x-1/4 before:top-0 before:h-px before:bg-primary/45',
    ...
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <Tooltip>
          <TooltipTrigger asChild>
            {hasPrevMonth ? (
              <Link
                href={prevHref}
                className={cn(navButtonClass, 'self-start')}
                aria-label={`Ir al mes anterior: ${prevMonthLabel}`}
              >
                <ChevronLeft className="h-4 w-4" aria-hidden data-icon="inline-start" />
                <span className="truncate">{prevMonthLabel}</span>
              </Link>
            ) : (
              <span
                className={cn(navButtonClass, 'pointer-events-none self-start opacity-45')}
        ...
        <div className="flex min-w-0 flex-1 flex-col items-center gap-2">
          <div className="inline-flex min-w-0 items-center gap-2 rounded-2xl border border-primary/40 bg-background/70 px-4 py-2 shadow-lg shadow-primary/10 ring-1 ring-primary/10">
            <CalendarDays
        ...
        {hasNextMonth ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <Link
                href={nextHref}
                className={cn(navButtonClass, 'self-end')}
                aria-label={`Ir al mes siguiente: ${nextMonthLabel}`}
              >
                <span className="truncate">{nextMonthLabel}</span>
          ...
          <div className="self-end">{nextFallbackAction}</div>
        )}
      </div>
    </div>
  );
